...
.. c:type:: MS_ARRAY
    ...
    .. c:member:: size_t Capacity
        ...
        Przykład automatycznej zmiany pojemności::
            ...
            ms_array_push_memval( &array, &list[0] );
            printf( "Length: %d -> Capacity: %d\n", array.Length, array.Capacity );
            ms_array_push_memval( &array, &list[1] );
            printf( "Length: %d -> Capacity: %d\n", array.Length, array.Capacity );

            /* w tym momencie zadziała inkreator */
            ms_array_push_memval( &array, &list[2] );
            printf( "Length: %d -> Capacity: %d\n", array.Length, array.Capacity );
    ...
    .. c:member:: size_t ItemSize
        ...
        jednak lepiej żyć w przekonaniu, że tablica taka może zawierać tylko elementy tego samego typu.
        Wartość ta nie powinna być zmieniana samodzielnie, gdyż może to prowadzić do błędów naruszenia ochrony pamięci
    ...
    .. c:member:: float Modifier
        ...
        .. sourcecode:: none
            ...
            Length: 1 -> Capacity: 1
            Length: 2 -> Capacity: 2
            Length: 3 -> Capacity: 55

    .. c:member:: bool Destroy

        Informacja o tym, zmienna przechowująca strukturę tablicy ma zostać zniszczona.
    ...
    .. c:member:: void** Items
        ...
        zmiennej z racji prostszej konstrukcji, iterując po niej w pętli.
    ...
    .. c:member:: size_t FuncIncrease(size_t capacity, float modifier)
        ...
        :param float modifier: Modyfikator zwracanej wartości.
...
.. c:type:: MSS_ARRAYFUNCTIONS
    ...
    Poniższe pola opisane są skrótowo z racji tego, iż schemat działania opisany jest dokładnie w polu
    do którego dana funkcja jest przeznaczona. Przykładem tego jest pole :c:member:`MS_ARRAY.FuncIncrease`.
    Aktualnie struktura pozwala na przechowanie tylko funkcji zwiększających pojemność tablicy.
    ...
    .. c:member:: size_t IncMultiply(size_t capacity, float modifier)
        ...
        :param size_t capacity: Aktualna pojemność tablicy.
        :param float modifier: Mnożnik pojemności.
    ...
    .. c:member:: size_t IncAdd(size_t capacity, float modifier)
        ...
        :param size_t capacity: Aktualna pojemność tablicy.
        :param float modifier: Składnik dodawania.
        :return: Nową pojemność tablicy przekazywaną do przydzielenia pamięci.
        :rtype: size_t
    ...
    .. c:member:: size_t IncPower(size_t capacity, float modifier)
        ...
        :param float modifier: Wykładnik potęgi.
        :return: Nową pojemność tablicy przekazywaną do przydzielenia pamięci.
        :rtype: size_t
...
.. c:var:: MSS_ARRAYFUNCTIONS MSC_ArrayFunctions
    ...
    Przykład użycia stałej::
        ...
        ms_array_push_memval( &array, &number );
...
.. c:function:: void* ms_array_alloc( size_t size, size_t capacity )
    ...
    :param size_t capacity: Początkowa ilość rezerwowanego miejsca na elementy tablicy.
...
.. c:function:: int ms_array_init( void *aptr, size_t size, size_t capacity )
    ...
    W przypadku błędu zwracany jest jego kod, w przeciwnym razie wartość ``MSEC_OK``.
    Przydzielone przez funkcję zasoby zawsze należy zwalniać, co umożliwia funkcja :c:func:`ms_array_free`.
...
.. c:function:: int ms_array_realloc( void* aptr, size_t capacity )
    ...
    .. sourcecode:: none
        ...
        Capacity: 30 <- Exact
        Error! MSEC_INVALID_ARGUMENT, FuncIncrease is missing!
        Capacity: 30

    :param aptr: Wskaźnik na tablicę.
    :param size_t capacity: Nowa pojemność tablicy lub 0 w przypadku automatu.
    :type aptr: void*
    :return: Kod błędu lub wartość ``MSEC_OK``.
    :rtype: int
...
.. c:function:: int ms_array_copy( void* adst, const void* asrc )
    ...
    W przeciwnym przypadku wszystkie dane zostaną nadpisane, co może się skończyć wyciekiem pamięci.
    ...
    Zerowanie danych również nie pomoże w każdym przypadku, gdyż wartość *NULL* nie zawsze składa się z
    ...
    Przykład użycia funkcji::
        ...
        MS_ARRAY array2 = ms_array_return_local( sizeof(int), 10 );
        MS_ARRAY array3;
        int      ercode;
...
.. c:function:: void* ms_array_copy_alloc( const void* aptr )
    ...
    wskaźniki do danych, kopiowane są tylko wskaźniki, więc dwie tablice będą miały dostęp
...
.. c:function:: int ms_array_insert_value( void* aptr, size_t index, const void* item )
    ...
    Dodaje do tablicy element, przekazany przez wskaźnik, we wskazane miejsce.
    ...
    znajdujących się za wartością ``index`` o jedno miejsce do przodu.
...
.. c:function:: int ms_array_insert_values( void *adst, size_t index, const void *tsrc, size_t count )
    ...
    Dodaje do tablicy elementy, przekazane w parametrze, do wybranego miejsca.
    ...
    Kopiowane elementy w tym przypadku musą być przekazane w postaci standardowej tablicy języka *C*.
...
.. c:function:: int ms_array_join_slice( void* adst, const void* asrc, size_t offset, size_t count )
    ...
    Podczas łączenia tablic elementy wstawiane są zawsze na samym końcu pierwszej tablicy.
    ...
    Przykład użycia funkcji::
        ...
        if( (ercode = ms_array_join_slice(&array2, &array1, 6, 0)) )
            printf( "Error! Function failed with code: %d!\n" );
    ...
    :param size_t offset: Indeks od którego wartości mają być kopiowane.
    :param size_t count: Ilość elementów w zakresie lub 0.
    :type adst: void*
    :type asrc: const void*
    :return: Kod błędu lub wartość ``MSEC_OK``.
    :rtype: int
...
.. c:function:: int ms_array_join_slice_inverse( void* adst, const void* asrc, size_t offset, size_t count )
    ...
    Dodaje do tablicy elementy z innej tablicy, pomijając wartości w podanym zakresie.
    Jak sama nazwa wskazuje, funkcja jest inwersją funkcji :c:func:`ms_array_join_slice`, co oznacza,
    że dodaje elementy spoza zakresu.
    Tak jak w przypadku wymienionej przed chwilą funkcji, wszystkie kopiowane elementy umieszczane są
    w tablicy na samym końcu.
    W przypadku podania wartości 0 do zmiennej ``count``, ilość kopiowanych elementów jest obliczana
    automatycznie i przyjmuje wartość równą ilości pozostałych elementów do końca tablicy, licząc od
    wartości zmiennej ``offset``.
    ...
    :param adst: Wskaźnik na tablicę do której elementy będą wstawiane.
    :param asrc: Wskaźnik na tablicę z której elementy będą kopiowane.
    :param size_t offset: Indeks od którego wartości nie będą kopiowane.
    :param size_t count: Ilość elementów w zakresie lub 0.
    :type adst: void*
    :type asrc: const void*
    :return: Kod błędu lub wartość ``MSEC_OK``.
...
.. c:function:: int ms_array_slice( void* aptr, size_t offset, size_t count )
    ...
    Wszystkie elementy spoza podanego zakresu są usuwane.
    Podanie wartości zmiennej ``offset`` innej niż 0, powoduje przesunięcie wszystkich elementów w zakresie
    i wyrównanie ich do indeksu zerowego.
    ...
    :param aptr: Wskaźnik na tablicę.
    :param size_t offset: Indeks od którego wartości nie będą usuwane.
    :param size_t count: Ilość elementów w zakresie lub 0.
    :type aptr: void*
    :return: Kod błędu lub wartość ``MSEC_OK``.
    :rtype: int
...
.. c:function:: void* ms_array_remove_range( void* aptr, size_t offset, size_t count )
    ...
    :param aptr: Wskaźnik na tablicę.
    :param size_t offset: Indeks od którego wartości będą usuwane.
    :param size_t count: Ilość elementów do usunięcia z tablicy lub 0.
    :type aptr: void*
    :return: Kod błędu lub wartość ``MSEC_OK``.
    :rtype: int
...
.. c:function:: void ms_array_clear( void* aptr )
    ...
    Przykład użycia funkcji::
        ...
        printf( "Elements: %d >> Capacity: %d\n", array.Length, array.Capacity );
        ms_array_push_values( &array, 0, list, 10 );
        printf( "Elements: %d >> Capacity: %d\n", array.Length, array.Capacity );
...
.. c:macro:: type ms_array_get( void* array, type type, size_t index )
    ...
    .. sourcecode:: none
        ...
        INDEX: 0 with VALUE: 630
        INDEX: 1 with VALUE: 342
        INDEX: 2 with VALUE: 534
        INDEX: 3 with VALUE: 678
        INDEX: 4 with VALUE: 944
...
.. c:macro:: type ms_array_getl( local array, type type, size_t index )
    ...
    :param local array: Tablica z której element ma być pobrany.
    :param type type: Typ pobieranego elementu.
    :param size_t index: Indeks elementu do pobrania.
    :return: Element pobrany z tablicy o typie podanym w parametrze.
    :rtype: type
